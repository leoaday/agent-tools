...
|                                                          |
|  +-----------+  +-----------+  +----------+  +--------+  |
|  |Claude Code|  | CodeBuddy |  | OpenCode |  | Others |  |
|  +-----+-----+  +-----+-----+  +----+-----+  +---+----+  |
|        |              |              |             |       |
|        v              v              v             v       |
|  +--------------------------------------------------+     |
|  |              Hook / Plugin 层                      |     |
|  |  (各Agent的hook脚本，拦截生命周期事件)              |     |
|  +-------------------------+------------------------+     |
|                            |                              |
|                            v                              |
|  +--------------------------------------------------+     |
...
│   │   └── report.js             # report命令 - 手动上报
│   ├── detector/
│   │   ├── index.js              # Agent检测协调器
│   │   ├── claude-code.js        # Claude Code检测 & 配置
│   │   ├── codebuddy.js          # CodeBuddy检测 & 配置
│   │   ├── opencode.js           # OpenCode检测 & 配置
│   │   ├── copilot-cli.js        # GitHub Copilot CLI检测 & 配置
│   │   ├── cursor.js             # Cursor检测 & 配置
│   │   ├── continue-cli.js       # Continue CLI检测 & 配置
│   │   ├── amazon-q.js           # Amazon Q检测 & 配置
│   │   └── aider.js              # Aider检测 & 配置
│   ├── hooks/
...
│       └── config.js             # 客户端配置管理
├── scripts/
│   └── postinstall.js            # npm postinstall (轻量检测+提示)
└── hooks/                        # 预构建的hook脚本文件
...
```json
{
  "version": 1,
  "hooks": {
    "sessionStart": [{ "type": "command", "bash": "node /path/to/universal-hook.js --agent=copilot --event=sessionStart" }],
    "postToolUse": [{ "type": "command", "bash": "node /path/to/universal-hook.js --agent=copilot --event=postToolUse" }]
  }
}
```
...
- **前端**：轻量SPA (Vue3/React 或纯ECharts Dashboard)
- **认证**：API Key (客户端上报) + 可选的Web登录
...
Aider无hook无MCP，采用命令包装方式：

```bash
# 安装后提供包装命令
agent-tools wrap-aider
# 实际上创建一个wrapper脚本，在aider执行前后采集数据
# 并解析 .aider.chat.history.md 提取使用信息
```
